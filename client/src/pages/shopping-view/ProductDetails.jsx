  ...
  useEffect(() => {
    async function fetchProductAndRecommendations() {
      ...
      try {
        ...
        const recsRes = await axios.get(`${BASE_API_URL}/api/shop/recommendations/${productId}`);
        if (recsRes.data.success) {
          setRecommendations(recsRes.data.data); 
        } else {
          console.warn("No specific recommendations returned or an issue occurred:", recsRes.data.message);
          setRecommendations([]); 
        }
